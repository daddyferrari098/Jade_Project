...
public class ContractNetVente extends ContractNetResponder {
    ...
    protected ACLMessage handleAcceptProposal(ACLMessage cfp, ACLMessage propose, ACLMessage accept) throws FailureException {
        ...
        if (liste != null) {
            ...
            //vérification et décrémentation nombre de places lors d'un achat
            liste.forEach(j->{
                var l =  catalog.getJourneysFrom(j.getStart());
                var l2 = l.stream().filter(j2->j2.getDepartureDate()==j.getDepartureDate() && j2.getStop().equals(j.getStop()) && (j2.getPlaces() > 0));
                var j3 = l2.toList().get(0);
                j3.setPlaces(j3.getPlaces()-1);
            });
            window.println("  !!!!");
