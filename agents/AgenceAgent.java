        ...
        addBehaviour(new CyclicBehaviour() {
            ...
            public void action() {
                ...
                if (msg != null) {
                    ...
                    //gestion des alertes signalés
                    //on met le nombre de places à 0 pour empêcher un achat sur les trajets alertés
                    String[] villes = msg.getContent().split(",");
                    var l = catalog.getJourneysFrom(villes[0].toUpperCase());
                    var l2 = l.stream().filter(j->j.getStop().equals(villes[1].toUpperCase()));
                    l2.forEach(j->j.setPlaces(0));
                } else {
                    block();
                }
            }
